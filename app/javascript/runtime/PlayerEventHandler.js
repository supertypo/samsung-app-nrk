...
PlayerEventHandler.restartPlayback = function() {
	if (Player.state != Player.STOPPED) {
		...
		if(PlayerEventHandler.retryCount <= Config.PLAYER_MAX_RETRIES) {
			Logger.log("Restarting playback, attempt " + PlayerEventHandler.retryCount + "/" + Config.PLAYER_MAX_RETRIES);
			if (PlayerEventHandler.currentSeconds > Config.PLAYER_JUMP_DELAY_SECONDS) {
...
PlayerEventHandler.jumpToPreviousLocation = function() {
	if (PlayerEventHandler.autoJump) {
		...
		}, Config.PLAYER_JUMP_DELAY_SECONDS * 1000);
	}
};
